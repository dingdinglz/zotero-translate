...
Smart Paper Translator 0.1.6 是面向 Zotero 9 内置 PDF 阅读器的学术翻译插件。它会在原生文本选择弹窗中直接显示缓存译文，未命中时可按设置手动或自动翻译，并按论文保存摘要、术语、句段译文和智能标签。

## 功能

- DeepSeek 内置配置，以及标准 Bearer 鉴权的 OpenAI Chat Completions 兼容服务。
- 两套可编辑安全模板；划线翻译模板默认携带论文标题与 Zotero 父条目摘要。
- 打开 PDF 时自动翻译尚未缓存的摘要；划线缓存命中时直接显示，未命中时默认等待点击“翻译”，也可在设置中开启划线后自动翻译。
...
- 划线弹窗会先在本机检查当前配置对应的缓存；命中时直接显示且不联网。未命中时，自动模式关闭则只有点击“翻译”后才发送，打开则划线后立即发送。
