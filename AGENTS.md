...
- 内置 Agent 只允许 `codex` / `pi` / `opencode`；Codex/Pi 适配器分别固定为 `@agentclientprotocol/codex-acp@1.6.2` / `pi-acp@0.0.33`。只有设置页“准备并检测”可联网准备，聊天启动必须使用离线 npm 模式，不得静默下载或升级。
- OpenCode 直接使用已选本机绝对路径运行 `opencode acp`，无需 Node/npx；以 1.18.30 为验证基线，只允许不低于它的稳定版 1.x，并验证 ACP 1、agentInfo 版本与 loadSession/session.close 能力。公共 Node 若已选择，目录仍排在子进程 PATH 首位，但不得成为 OpenCode 检测或启动的必填条件。禁用自动升级与模型目录后台刷新，npm 保持 offline；依赖缺失时提供终端准备说明，不静默下载。
- OpenCode 自带 HTTP 服务必须显式绑定 `127.0.0.1`、随机端口、关闭 mDNS；随机认证凭据仅存在于当前进程环境，不写入参数、偏好、目录缓存或日志。检测使用独立连接、临时数据库和空 session，不发提示词；结束时关闭 session、进程并清理插件自己的临时数据，不删除用户会话。清理与重新启动必须串行，不能让旧清理删除新工作区。
- OpenCode `model` 保留完整模型 ID，`effort` 映射思考强度，动态 `mode` 显示为“运行模式”并保存到当前论文的 `session.config.agentMode`，与 Codex 权限 `mode` 分开。新会话默认跟随本机模式；选项完全来自 ACP，变更确认成功后保存，失败恢复原值；恢复先应用模型再校验思考档位。授权沿用本机规则并通过 ACP 交互卡片明确选择。
- OpenCode 检测有界读取 `models --verbose`，输出最多 16 MiB、模型最多 10000；仅保留模型 ID 与明确的图片/PDF 输入能力，禁止持久化原始输出、provider headers/options 或密钥。路径或本机版本变化必须失效检测缓存，保留历史。模型没有明确图片能力时阻止发送并保留完整截图草稿；没有原生 PDF 能力时提供工作区 source.txt 引用。回放跳过合成上下文及类型化 PDF/图片块，只通过已验证截图 ID 关联本地副本。
- OpenCode 禁用终端标题和彩色输出；兼容本机扩展仍写入 stdout 的 OSC 0/1/2 标题控制码，流式过滤必须处理分块，单序列最多 4096 字节，拒绝未闭合、其他控制码及普通非 JSON 日志，不得放宽 Codex/Pi JSON 校验。只读 `models --verbose` 使用固定 `/bin/sh` 收集脚本：路径仅经 argv 传递，清除 ENV/BASH_ENV，收到随机结束标记后才关闭 stdin 释放管道，避免 Gecko HUP 提前截断；保持退出码和输出/超时上限，取消时杀死并回收所属子进程。不得执行用户 shell 启动文件、下载依赖或将原始输出写入文件。ACP 会话仍直接启动本机 OpenCode。
...
│       ├── preferences.xhtml         # 翻译/公共 ACP 环境、路径候选/编辑/浏览、三 Agent Tab 与开发者模式
